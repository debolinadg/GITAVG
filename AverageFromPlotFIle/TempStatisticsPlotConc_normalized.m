...
for i = 1 : 1 : length(Yplots)
    ...
    for jj = 1 : 1 : length(Ka)
        plot(BinTemp(:,jj),(BinX(:,Yplots(i),jj)*(101325/8.314)./BinTemp(:,jj))/max((BinX(:,Yplots(i),jj)*(101325/8.314)./BinTemp(:,jj))),'LineWidth',3)
        hold on
    end
    plot(Temp0,(X0(:,Yplots(i))*(101325/8.314)./Temp0)/max((X0(:,Yplots(i))*(101325/8.314)./Temp0)),'--k','LineWidth',3)
    plot(Tempm,(Xm(:,Yplots(i))*(101325/8.314)./Tempm)/max((Xm(:,Yplots(i))*(101325/8.314)./Tempm)),':k','LineWidth',3)
    hold on
